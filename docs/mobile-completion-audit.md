...
| Gamified resident workflow | `/collection` shows level, XP, score, badges, and private collection cards; `/leaderboard` ranks resident collectors; only cleared captures count toward points, levels, badges, and rankings, while pending and rejected captures remain in private history without scoring. |
